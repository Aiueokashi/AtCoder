...
struct Combination{
    ...
    Mint second_stirling_number(ll n, int k){ //n個の区別できるボールをk個の区別できない箱に分割
        Mint ret = 0;
        rep(i, k+1){
...
int main(){
    INT(N, M);
    vec2(mint, dp, 501, M+1, 0);
    dp[0][0] = 1;
    rep(i, 500){
       rep(j, M+1){
           dp[i+1][j] = dp[i][j];
           int k = (i+1)*(i+2)/2;
           if(j-k >= 0) dp[i+1][j] += dp[i+1][j-k];
       }
    }
    mint ans = 0;
    vec1(mint, memo, M+1, 0); vec1(mint, sum, M+1, 0);
    memo[0] = 1;
    rep2(l, 0, 500){ //左l個
        int r = min(500, N-l-1); //右r個以下
        if(r < 0) break;
        if(l > 0){
            rep(i, M+1) memo[i] = dp[l][i]-dp[l-1][i];
        }
        rep3(i, M, 0){
            sum[M-i] = dp[r][M-i];
            if(M-i-N >= 0) sum[M-i] += sum[M-i-N];
            ans += memo[i]*sum[M-i];
        }
    }
    out(ans);
