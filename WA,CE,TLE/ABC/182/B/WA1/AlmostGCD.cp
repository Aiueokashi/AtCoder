...
inline Point operator*(DD a, const Point &p) {
...
inline Point rot(const Point &p, DD ang) {
	return Point(
		cos(ang) * p.x - sin(ang) * p.y, sin(ang) * p.x + cos(ang) * p.y);
}
...
inline DD dot(const Point &p, const Point &q) {
	return p.x * q.x + p.y * q.y;
}
inline DD norm(const Point &p) {
	return dot(p, p);
}
inline DD abs(const Point &p) {
	return sqrt(dot(p, p));
}
...
//演算系
vector<int> cumulative(vector<int> a){
vector<int> s(len(a)+ 1, 0); 
for (int i = 0; i < len(a); ++i) s[i+ 1] = s[i] + a[i];
return s;
}
...
bool is_contain(
	...
	const Point &b,
	const Point &c) {
	int r1 = simple_ccw(p, b, c), r2 = simple_ccw(p, c, a),
		r3 = simple_ccw(p, a, b);
	if (r1 == 1 && r2 == 1 && r3 == 1)
		return true;
	if (r1 == -1 && r2 == -1 && r3 == -1)
...
vout(gcddo);
gcddo[0]=0;
vector<int>::iterator iter = max_element(gcddo.begin(), gcddo.end());
  size_t index = distance(gcddo.begin(), iter);
  cout << index+ 2 << endl;
}
